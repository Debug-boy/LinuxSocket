...
namespace infinity::socket{
        ...
        [[maybe_unused]] ssize_t send(int32_t socket_fd,void* buffer,size_t length){
            ...
            while(length){
                size_t real_length = ::send(socket_fd,local_buffer,length,0);
                if(real_length < 0)return socket::SOCKET_ERROR;//"unknow exception!"
                else if(real_length == 0)return socket::SOCKET_CLOSE;//target is close scoket!
